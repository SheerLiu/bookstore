...
<%Book books=(Book)request.getAttribute("book");%> 
					<img src="../<%=books.getimage_path()%>" alt="">
					<div class="infobookt">
		<% out.println("<h3>"+books.getbookname()+"</h3><br>");%>
						<ul>	
					
		<% out.println("一间价：￥"+books.getprice_current()+"<br>");%>
		<% out.println("<del>"+"定价：￥"+books.getprice_original()+"</del><br>");%>
		<% out.println("<strong>"+"库存："+books.getnum_stock()+"</strong>"+"<br>");%>
							<li>库存：<strong>暂时缺货</strong></li>
<% out.println("作者："+books.getauthor()+"<br>");%>
<% out.println("出版社："+books.getpress()+"<br>");%>
<% out.println("出版时间："+books.getdate()+"<br>");%>
<% out.println("页数："+books.getnum_page()+",");%>
<% out.println("字数："+books.getnum_word()+"<br>");%>						

							<li>纸张：胶版纸</li>
<% out.println("ISBN："+books.getisbn()+"<br>");%>
							<li>包装：平装</li>
						</ul>
						<div><a href="accordion.html" class="more">试读</a>&nbsp;
			<a href="addShop?name=<%=books.getbookname()%>" class="cart">加入购物车</a>
						</div>
						</dit>
				</section>
				<section class="infobookc">
					<h3>编辑推荐</h3>
					<% out.println(books.getcomment()+"<br>");%>
					<h3>内容简介</h3>
					<% out.println(books.getintro_content()+"<br>");%>
					<h3>作者简介</h3>
					<% out.println(books.getintro_author()+"<br>");%>
				</section>
			</section>
